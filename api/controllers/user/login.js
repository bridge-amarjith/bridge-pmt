...
module.exports = {
  ...
  fn: async function (input, exits) {
    try {
      const user = await Pmt_users.find({ email: input.email }).limit(1);
      if (user == "") return exits.notUser({
        error: `Account belongs to ${input.email} not found`
      })
      await Pmt_users.comparePassword(input.password, user[0].password)
      ...
      return exits.error({
        message: `Error logging in user ${input.email}`,
        error: e.message,
      });
    }
  }
